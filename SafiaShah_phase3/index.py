...
# Spring 2022
# Objective: Phase 3 of multiphase project
...
class linkedList:
    ...
    def traverse(self):
        curr=self.head
        while curr!=None:
            print("Doc: {:<3}".format(curr.doc), " Freq: {:<6}".format(curr.freq), " wt: {:<5f}".format(curr.weight),end= " ")
            curr=curr.next
    ...
    def computeWts(self, corpusNum, term_dict, df):# word):
        ...
        while curr!=None:
           ...
           # if (word == "textplain"):
            ...
            curr=curr.next
    def writePostingsFile(self, output_dir, word,f): 
        curr=self.head
        while curr!=None:
            f.write("{} {},{:.8f}\n".format(word,curr.doc,curr.weight)) #for debugging/ confirming correct output
           # f.write("{},{:.8f}\n".format(curr.doc,curr.weight))
            curr = curr.next

    def writeDictFile(self,output_dir, item,f, count): ###################################
        
        curr=self.head
        while curr!=None:
            #f.write("{}\n{:.8f}\n ".format(curr.doc,curr.weight)) ##need to figure out how to add position in postings file
            curr = curr.next
       

# write the outputs of the program - dictionary and the posting index
def writeOutput(H_DICT, output_dir):
    #write postings file
    with open("{}/postings_file.txt".format(output_dir), "w") as f: 
        for item in H_DICT:
            H_DICT[item].writePostingsFile(output_dir, item,f)
    f.close()

    #write dictionary file
    postingsIndex = 1
    with open("{}/dictionary.txt".format(output_dir), "w") as f: 
        for item in H_DICT:
            count = H_DICT[item].countList()
            f.write("{}\n{}\n{}\n".format(item,count, postingsIndex))
            postingsIndex += count
           # H_DICT[item].writeDictFile(output_dir, item, f, count)
    f.close()

#------------------------------------------printing dictionary functions---------------------------------
...
def editDict(H_dict, term_dict):
    ...
    term_array = []
    #print_LLDict(H_dict)
    #go through full corpus and get rid of buckets whose terms only have 1 file and the word freq
    # in that file is 1 associated with them
    for word in H_DICT:
...
def main():
   ...
   # print_LLDict(H_DICT) 
    ...
    # write doc id and weight for each posting into posting file doc as well as dictionary
    writeOutput(H_DICT, output_dir)
